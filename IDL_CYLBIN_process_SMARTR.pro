...
;PRO GO, $
pro IDL_CYLBIN_process_SMARTR_updated
 
  
  CLOSE, /all
  @idl_cylbin_params.pro

  IF (N_ELEMENTS(divergence_range) EQ 0) THEN divergence_range = [-3.0,3.0]
  ...
  OPENR,2,FileNameList                                                ;;; Grab all in list
...
;;; it from the code later, for now let's just label is more clearly
  ...
  vr_guess = FLTARR(naz,nr,nz)        ; Not used? ABF
  ...
  ;Oct 29, 2015 rain_1km_GATEZR  = FLTARR(nt,nr)
  ;Oct 29, 2015 rain_1km_localZR = FLTARR(nt,nr)
  ...
  ;IF (PLOT_OUTPUT_DEVICE GE 2) THEN BEGIN
  ;   SET_PLOT, 'PS'
  ; margin     = 2.54
  ; page_width = margin*8.5
  ; page_height= margin*11.0
  ; xoffset   = margin
  ; yoffset   = margin
  ; xsize      = page_width  - 2.0*margin
  ;   ysize      = page_height - 2.0*margin
  ;   DEVICE, FILENAME=file_plot
  ; DEVICE, XSIZE=xsize, YSIZE=ysize, XOFFSET = xoffset, $
  ;           YOFFSET = yoffset, /COLOR
  ;ENDIF
  ...
  ;;; of dBZ (called Zhist), Vr (called Vhist) and spectral width
  ...
  for ihr=0,nt-1 do begin
    ...
    ; OPENR,1,filebase+'.Zhist', SWAP_ENDIAN=NEED_TO_SWAP_ENDIAN_FLAG
    ; READU,1,dbzhist
    ; CLOSE,1
    ;
    ; OPENR,1,filebase+'.Vhist', SWAP_ENDIAN=NEED_TO_SWAP_ENDIAN_FLAG
    ; READU,1,vrhist
    ; CLOSE,1
    ;
    ; OPENR,1,filebase+'.Whist', SWAP_ENDIAN=NEED_TO_SWAP_ENDIAN_FLAG
    ; READU,1,widhist
    ; CLOSE,1
    ;
    ; OPENR,1,filebase+'.Vsum', SWAP_ENDIAN=NEED_TO_SWAP_ENDIAN_FLAG
    ; READU,1,vrsum
    ; CLOSE,1

    dbzhist = READ_BINARY(filebase + '.Zhist', DATA_DIMS=[ndbz,naz,nr,nz], DATA_TYPE=15)
    vrhist  = READ_BINARY(filebase + '.Vhist', DATA_DIMS=[nvr,naz,nr,nz], DATA_TYPE=15)
    widhist = READ_BINARY(filebase + '.Whist', DATA_DIMS=[nwid,naz,nr,nz], DATA_TYPE=15)
    vrsum = READ_BINARY(filebase + '.Vsum',  DATA_DIMS=[naz,nr,nz], DATA_TYPE=4)
...
;Oct29,2015             nobsSDZ(*,ira)=nobsSDZ(*,ira) $
;Oct29,2015                             +total(total( fSDZhist(1:*,2:*,*,ira) ,2),1) ;;; loop over iaz weighted-summing the histogram of non-missing values
;Oct29,2015              for iaz = 0,naz-1 do $
;Oct29,2015                 SDZ(iaz,ira)=SDZ(iaz,ira) + $
;Oct29,2015                 total(SDZ_values* total(fSDZhist(1:*,2:*,iaz,ira),2) ,1)
;Oct29,2015    
;          
           endfor ;;ira1
        endfor    ;;ira
...
;Oct29,2015        SDZ = SDZ/(nobsSDZ > 1)
        
;;; Declare MISSING (-9999) if there are < min_obs (defined in params). 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; p loop
;      for ip=0,np-1 do begin
        for ip=0,np-1 do begin ;;; For debugging, use IP_FIRST ;;;; nov 04, 2015
           iz=ip2iz(ip)
...
;;; skip if empty
                 ...
                 if(Vr_p(iaz,ira,ip) lt -9990.0 or nobs_temp lt nobs_min) then begin
                   ...
                   ;;; comment nov 13 print, 'No good data - ', Vr_p_temp, ' nobs ', nobs_temp
                    goto,jump22
                 endif
...
;;; Commented start from here Nov 05, 2015;;;;
...
;;; Commented end from here Nov 05, 2015;;;;
...
;comment Nov 13                          if(Vr1(iaz) lt -9990.0) then goto,jump27 ;; bail out
...
;;; radius. These come from dividing the sdbzhist across the 4:1 line
 ...
 ; Oct29,2015      strat_dbzs = total( fsdzhist*strat_mask, 1) ;; result is a 66, 24, 12 array (dbz,az,range)
 ; Oct 29,2015      conv_dbzs = total(  fsdzhist* conv_mask, 1) ;; result is a 66, 24, 12 array (dbz,az,range)
 
;;; Convert dbZ to rainrate
; Oct29,2015        stratrain = rrate_GATEZR(2:*,*,*)*strat_dbzs(2:*,*,*) 
; Oct29,2015        convrain = rrate_GATEZR(2:*,*,*)*conv_dbzs(2:*,*,*) 
; Oct29,2015        stratrainrate = total(stratrain(*,*,*),1) ;;; sum over dBZs, a 2D (az, range) map of rainrate        
; Oct29,2015        convrainrate = total( convrain(*,*,*),1)  ;;; sum over dBZs, a 2D (az, range) map of rainrate 
        
;;; Now average over area within each ccircle of radius iira
 ; Oct29,2015       for iira= 0, n_elements(r)-1 do $
 ; Oct29,2015          srain =   total( (stratrainrate*area2d)[*,0:iira] ) / $
 ; Oct29,2015         total( (         area2d)[*,0:iira] )    ;;; mm/hr within radii
  ; Oct29,2015      for iira= 0, n_elements(r)-1 do $
  ; Oct29,2015         crain =   total( ( convrainrate*area2d)[*,0:iira] ) / $
  ; Oct29,2015         total( (         area2d)[*,0:iira] )    ;;; mm/hr within radii
        
        
        
;;; Now ignore the absolute values and just redistribute C-S fraction
 ; Oct29,2015       sfrac = srain/( (srain+crain)>1e-10)
;;; Save this off in the dataset d01, at the appropriate pressure level 
; Oct29,2015        d01(29,*,plotalt,ihr)=sfrac ;;; mm/h
...
;;; Joint histogram
; Oct29,2015            jhist = total(total( sdzhist, 4),3)
; Oct29,2015            contour, jhist(1:*,2:*), sdz_values(1:*), dbz(2:*), $
; Oct29,2015                     xtit='dBz gradient', ytit='dBZ', /fill, nlev=23, tit='SDZ - dbZ joint histogram, 2-4km layer'
; Oct29,2015            oplot, [0,20], [0,80] ;;; suggested line for strat-conv
;;; Joint histogram (short ranges only)
; Oct29,2015            jhist = total(total( sdzhist(*,*,*,0:nr/2), 4),3)
; Oct29,2015            contour, jhist(1:*,2:*), sdz_values(1:*), dbz(2:*), $
; Oct29,2015                     xtit='dBz gradient', ytit='dBZ', /fill, nlev=23, tit='joint histogram inside r< rmax/2'
; Oct29,2015            oplot, [0,20], [0,80] ;;; suggested line for strat-conv
...
; Oct29,2015     stratiform_fraction(ihr,*)=d01(29,*,3,ihr)
; Oct29,2015      rain_plotalt_localZR(ihr,*)=d01(6,*,3,ihr)
     cfad0(*,*,ihr)=cfad_p
